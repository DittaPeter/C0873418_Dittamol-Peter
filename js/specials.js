$(document).ready(function() {
    ...
    $("#seach-button").click(function () {
      ...
      if (!searchTerm) {
        ...
        var flickrApiUrl = 'https://api.flickr.com/services/feeds/photos_public.gne?format=json&jsoncallback=?&tags=' + searchTerm;
        $.ajax({
          ...
          success: function(responseData) {
            if (responseData && responseData.items) {
              ...
              (responseData.items).forEach(function(item, i) {
                if (i < 6) {
                  divData += "<div class=image-flickr><h2>" + item.title + "</h2><img src=" + item.media.m + "><div class=tag>" + item.tags + "</div></div>";
                }
              });
              $("#images-flickr").html(divData);
